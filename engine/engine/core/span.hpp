...
{
    T t = narrow_cast<T>(u);
    if (static_cast<U>(t) != u) FATAL( "Fail to perform narrow cast" );
    if (!details::is_same_signedness<T, U>::value && ((t < T{}) != (u < U{})))
        FATAL( "Fail to perform narrow cast" );
    return t;
}
...
// Specialization of at for span
template <class ElementType, std::ptrdiff_t Extent>
constexpr ElementType& at(span<ElementType, Extent> s, std::ptrdiff_t i)
{
