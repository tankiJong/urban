...
using Event = async_manual_reset_event;
...
public:
   ...
   virtual async_generator<BaseJob*>  Request( eWorkerThread id, Event& runningEvent ) = 0;
   static void       Init();
   static void       Cleanup();
   static Scheduler& Get();
   static task<>       Wait( JobHandleArray& jobs );
   ...
   {
   public:
      TriggerEventJob( Event* eve )
         : mEvent( eve ) {
         ASSERT_DIE( eve );
      }

      void operator()( eWorkerThread )
      {
         mEvent->set();
      }
      Event* mEvent;
   ...
   template<typename F, typename T = std::invoke_result_t<F, eWorkerThread>>
   static task<T> CreateAndDispatchValueJob( span<JobHandleRef> dependencies, F&& func, eWorkerThread threadMask = ~0 )
{
   struct ValueJob
   {
      ValueJob( F&& func, T& value, Event& trigger )
   ...
   Job<ValueJob>::Allocate( dependencies, threadMask ).InitAndDispatch( std::forward<F>( func ), value, trigger );
