...
#include <stdlib.h>
#include <string.h>
#include "engine/scripting/es.hpp"
   ...
   char*         argv[] )
{
   for(int i = 1; i < argc; i++) {
      ...
      if(strcmp( str, "--shell" ) == 0) { run_shell = true; } else if(strcmp( str, "-f" ) == 0) {
...
int main( int argc, char* argv[] )
{
   ...
   {
      auto context = CreateShellContext( isolate );
   ...
   Script script;
   Script::FromFile( script, "hello.js" );
   auto s = script.Handle().Get( isolate );
   s->BindToCurrentContext()->Run( context );
   while(1) {
      using namespace v8;
   }
   es::Shutdown();
